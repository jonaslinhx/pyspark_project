from pyspark.sql.functions import (
    struct, lit, col, array, when, isnull, 
    filter, current_timestamp, expr,
...
def get_contract(df):
    "Take account dataframe and convert to dataframe of triplets"
...
def get_relations(df):
    "Take party dataframe and create triplets"
    return df.select(
        "account_id", "party_id",
...
def get_address(df):
    "Take address dataframe and convert to triplet"
    address = struct(
...
def apply_header(spark, df):
    "takes final df and adds event header to each record"
    header_info = [("pyspark_project", 1, 0), ]
    header_df = spark.createDataframe(header_info).toDF("eventType", "majorSchemaVersion", "minorSchemaVersion")
